...
class TD7HockyAgent(HockeyAgent):
    def __init__(
        ...
        config: CfgNode = None,
        model: TD7 = None,
        trainEnv: gym.vector.AsyncVectorEnv = None,
        evalEnv: HockeyEnv_SB3 = None,
        loadModel: bool = None,
        modelsDir: str = None,
        modelName: str = None,
    ):
        if not loadModel:
            ...
            self.saveDir = os.path.join(
                self.config.agent.save_dir,
                config.model.name,
                f"{self.config.train_env.env_name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
            )
            ...
            if self.config.agent.save:
                self.saveDir = os.path.join(
                    modelsDir,
                    "td7",
                    f"{modelName}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
                )
                self.writer = SummaryWriter(log_dir=self.saveDir)
            else:
                self.writer = None
    ...
    def train(self, *args, **kwargs):
        ...
        while totalTimesteps < trainingConf.total_timesteps:
            ...
            if totalTimesteps >= learningStarts:
                ...
                actions = [
                    self.trainEnv.single_action_space.sample() for _ in range(numEnvs)
                ]

            nextStates, rewards, dones, _, _ = self.trainEnv.step(actions)

            self.model.replayBuffer.add(
                states,
                actions,
                nextStates,
                rewards.reshape(-1, 1),
            ...
            if totalTimesteps >= learningStarts and not useCheckpoint:
                ...
                if self.writer is not None:
                    self.writer.add_scalar(
                        "Loss/Encoder", losses["encoder_loss"] or 0, totalTimesteps
                    )
                    self.writer.add_scalar(
                        "Loss/Critic", losses["critic_loss"] or 0, totalTimesteps
                    )
                    self.writer.add_scalar(
                        "Loss/Actor", losses["actor_loss"] or 0, totalTimesteps
                    )
                    self.writer.add_scalar(
                        "Reward/Average Step Reward", np.mean(rewards), totalTimesteps
                    )
            ...
            if (
                totalTimesteps >= trainingConf.eval_starts
                and totalTimesteps % trainingConf.eval_every == 0
            ):
                self.evalMidTrain(
                    timeStep=totalTimesteps,
                    episode=episodeNum,
                    evalEpisodeNum=trainingConf.eval_episode_num,
                )
            ...
            if np.any(dones):
                ...
                if totalTimesteps >= learningStarts and useCheckpoint:
                    self.model.maybeTrainAndCheckpoint(
                        epTimesteps=maxRewardTotalTimeStep, epReturn=maxEpTotalReward
                    )
    ...
    def load(self, path: str = None):
        ...
        dir = path or os.path.join(
            self.config.agent.save_dir,
            self.config.model.name,
            self.config.model.model_load_name,
        )
    ...
    def evalMidTrain(self, timeStep: int, episode: int, evalEpisodeNum: int):
        ...
        for _ in range(evalEpisodeNum):
            ...
            while not done:
                action = self.model.selectAction(
                    state[np.newaxis, :], useExploration=False
                )[0]
        ...
        print(
            f"Average total reward over {evalEpisodeNum} AVG episodes: {avgReward} STD: {stdReward}"
        )
        print("--------------------------------------------")
    ...
    def predict(self, obs, deterministic=True):
        action = self.model.selectAction(
            obs[np.newaxis, :], useExploration=not deterministic
        )[0]
        return action, None
...
class TD7GymAgent(TD7HockyAgent):
    def __init__(
        ...
        super().__init__(
            config, model, trainEnv, evalEnv, loadModel, modelsDir, modelName
        )
    ...
    def evaluate(
        ...
        stdReward = np.std(totalRewards)
        print(
            f"Average total reward over {evalConf.ep_num} episodes: {avgReward} STD: {stdReward}"
        )
        print("--------------------------------------------")
        if self.config.agent.save:
