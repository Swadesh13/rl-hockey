...
def plot_tensorboard_data(
    ...
    for experiment, (steps, values) in data.items():
        ...
        print(
            f"  {experiment}[{best_name}]: Best {metric} = {best_value:.2f} at step {best_step:,}"
        )
...
def plot_tensorboard_data_RND(
    ...
    for experiment, (steps, values) in data.items():
        color, label = custom_styles.get(experiment, (None, experiment))
        plt.plot(steps, values, label=label, color=color)
    ...
    sorted_pairs = sorted(
        zip(filtered_handles, filtered_labels), key=lambda x: sort_key(x[1])
    )
